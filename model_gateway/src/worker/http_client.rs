...
/// config. Entries are never evicted: the map is bounded by distinct configs,
/// not worker count, so worker removal cannot leak.
pub struct WorkerHttpClientCache {
    client_identity: Option<Vec<u8>>,
    ca_certificates: Vec<Vec<u8>>,
    upstream_http2: bool,
    request_timeout_secs: u64,
    clients: Mutex<HashMap<ClientKey, Arc<reqwest::Client>>>,
}
...
impl WorkerHttpClientCache {
    ...
    /// The shared client for a worker's effective pool config, built on first
    /// use.
